package com.example.Labe.SoapWs;
...
public class TestClientEndpointSoap {
    ...
    @Test
    public void testEnviarSoayRecibirSoap(){
        WebServiceTemplate ws = new WebServiceTemplate(marshaller);
        GetClienteRequest request = new GetClienteRequest();
        request.setId(1);
        GetClienteResponse response = (GetClienteResponse)ws.marshalSendAndReceive("http://localhost:" + port + "/ws",request);
        System.out.println("response: " + response.getCliente().getNombre());
        assertThat(response != null);
        assertThat(response.getCliente().getNombre() != "Leonel");
    }
